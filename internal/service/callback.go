...
var callbackMappersMap = map[string]*callbackMapper{
	...
	"aplex":   newCallbackMapper(aplexCallbackMapper, http.MethodPost),
}
...
func aplexCallbackMapper(payRepo *repos.Repo, gtwAdapterId string, content []byte, query string, headers http.Header) (int64, any, error) {
	logger := log.New("dev")
